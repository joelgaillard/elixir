...
const useLocationStore = () => {
  ...
  // Synchronisation des données avec `watch`
  watch(geoCoords, (newCoords) => {
    if (newCoords) {
      coords.value = { latitude: newCoords.latitude, longitude: newCoords.longitude }
    }
  })
  ...
  const locationReady = computed(() => coords.value.latitude !== null && coords.value.longitude !== null)
